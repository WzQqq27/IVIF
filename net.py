...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x)))   # 图中的“+”，缓解梯度消失
...
class CrossSwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, x_size):
        ...
        cross_attn_windows_A = self.attn_A(x_windows, y_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        cross_attn_windows_B = self.attn_B(y_windows, x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C

        # W-MSA/SW-MSA
        attn_windows_x = self.attn(cross_attn_windows_A, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        attn_windows_y = self.attn(cross_attn_windows_B, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        ...
        attn_windows_y = attn_windows_y.view(-1, self.window_size, self.window_size, C)
        ...
        # FFN
        x = shortcut_A + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))   # 图中的“+”，缓解梯度消失
        y = shortcut_B + self.drop_path(y)
        y = y + self.drop_path(self.mlp(self.norm2(y)))   # 图中的“+”，缓解梯度消失
...
class CrossSwin(nn.Module):
    def __init__(self, img_size=224, patch_size=1, in_chans=1, out_chans=1,
                 embed_dim=96, depths=[4, 2, 4], num_heads=[1, 2, 4],
        ...
        self.EN2_0 = CrossStageModule(dim=int(embed_dim),
                                 input_resolution=(patches_resolution[0],
                                                   patches_resolution[1]),
                                 depth=depths[1],
        ...
        self.conv = nn.Conv2d(in_channels=192, out_channels=self.out_chans, kernel_size=(1, 1), stride=(1, 1),padding=(0, 0))
        ...
        self.fusion_strategy =   SwinTransformerBlock(dim=192, input_resolution=(patches_resolution[0],patches_resolution[1]),
    ...
    def encoder(self, x, y):
        ...
        print("pos_drop",x.shape)  #[1,50176,96]
        ...
        x3, y3 = self.EN2_0(x2, y2, (224,224))
        x3 = x3 + x2
        y3 = y3 + y2
    ...
    def fusion(self, en1, en2):
        print("拼接的输入",en1.shape)
        # attention weight
        f = torch.cat([en1,en2], dim=1)
        print("concat后",f.shape)
        f = to_3d(f)
        f = self.fusion_strategy(f)
        f = to_4d(f,224,224)
        print("融合特征",f.shape)
    ...
    def up_x4(self, x):
        x = self.conv(x)
        # x = self.tanh(x)  #训练用
        x = self.sigmoid(x)   #测试用
        return x

    def finaldecoder(self, x, y):

        x3, y3 = self.encoder(x, y)
        # f = self.fusion(x3, y3, 'l1_mean')
        f = self.fusion(x3, y3)
        x = self.up_x4(f)
